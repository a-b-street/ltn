<script lang="ts">
  ...
  let addMode = $state<"choose-area" | "draw-area">("choose-area");
  ...
  let selectedBoundaries: Map<number, GeneratedBoundaryFeature> = $state(
    new Map(),
  );
  ...
  let drawnBoundary: GeneratedBoundaryFeature | null = $derived.by(() => {
    if (drawnShape && drawnShape.geometry.coordinates[0].length > 3) {
      ...
      try {
        return $backend!.generateMergedBoundary(featureCollection);
      } catch (error) {
  ...
  let neighbourhoodBoundary: GeneratedBoundaryFeature | null = $derived(
    addMode == "draw-area" ? drawnBoundary : selectedBoundary,
  );
