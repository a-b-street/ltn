...
export class RouteTool {
  ...
  active: boolean;
  eventListenersSuccess: ((f: Feature<LineString | Polygon>) => void)[];
  eventListenersUpdated: ((f: Feature<LineString | Polygon>) => void)[];
  ...
  finish() {
    ...
    if (rawJSON) {
      ...
      for (let cb of this.eventListenersSuccess) {
        cb(JSON.parse(rawJSON) as Feature<LineString | Polygon>);
      }
    } else {
